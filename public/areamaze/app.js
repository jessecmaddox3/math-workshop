(() => {
  ...
  async function main() {
    ...
    function newPuzzle() {
      ...
      el2("guess").value = "";
      el2("guess").disabled = false;
      el2("check").disabled = false;
      el2("tier").textContent = String(state.tier);
      el2("solved").textContent = String(state.solved);
      draw();
      el2("guess").focus();
      hintTimer = setTimeout(() => {
        el2("hint").textContent = puzzle.idea;
    ...
    function draw() {
      ...
      const clues = puzzle.given.map((ref) => {
        const [i, kind2] = ref.split(".");
        return `Rectangle ${Number(i) + 1} ${words[kind2]} ${valueOf(puzzle, ref)}`;
      });
      ...
      puzzle.cells.forEach((cell, i) => {
        ...
        div.innerHTML = `<span class="area${isAskArea ? " q" : ""}">` + (areaShown ? cell.a : isAskArea ? "?" : "") + "</span>";
    ...
    function check() {
      if (el2("guess").disabled) return;
      const want = answerOf(puzzle);
      const got = Number(el2("guess").value.trim());
      const fb = el2("feedback");
      if (!el2("guess").value.trim() || !Number.isFinite(got)) {
      ...
      if (got === want) {
        ...
        fb.innerHTML = `<span class="verdict">${want}. Correct.</span> <span class="gloss">${puzzle.idea}</span>`;
        el2("guess").disabled = true;
        el2("check").disabled = true;
        ...
        fb.innerHTML = tries >= 2 ? `<span class="verdict">Not ${got}.</span> <span class="gloss">${puzzle.idea}</span>` : `<span class="verdict">Not ${got}.</span> <span class="gloss">Look for two rectangles that share a side.</span>`;
      }
      fb.hidden = false;
    }
    el2("check").addEventListener("click", check);
    el2("guess").addEventListener("keydown", (e) => {
      if (e.key === "Enter") check();
    });
    el2("skip").addEventListener("click", newPuzzle);
    document.addEventListener("progress-loaded", () => {
      const fresh = readLocal(GAME);
      state.tier = fresh.tier;
      state.solved = fresh.solved;
      state.streak = 0;
      state.done = new Set(fresh.done);
