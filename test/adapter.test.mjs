...
function adapter(){
 ...
 vm.runInContext(source+`\nglobalThis.fixture={set(values){({profile,record,live,selectionEpoch,store,cloud,message}=values)},refresh:refreshActive,mutate:mutateActive,get:()=>({profile,record,live,retired})};`,context);
...
test('own cloud attachment adopts its revision without disconnecting or changing the question',async()=>{
 const {api,events}=adapter();const accepted={profileId:'Comet',localRevision:2,snapshot:{solved:3},binding:{ownerId:'synthetic'}};api.set({...current('Comet',1,3),store:{load:async()=>accepted}});
 await api.mutate(async()=>({status:'saved',record:accepted}));assert.equal(api.get().record.localRevision,2);assert.equal(api.get().retired,false);assert.deepEqual(events,[]);
});
test('choosing a different cloud snapshot resets the active controller to the accepted progress',async()=>{
 const {api,events}=adapter();const accepted={profileId:'Comet',localRevision:8,snapshot:{solved:4},binding:{ownerId:'synthetic'}};api.set({...current('Comet',7,12),store:{load:async()=>accepted}});
 await api.mutate(async()=>({status:'saved',record:accepted}));
